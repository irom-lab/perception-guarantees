...
def main(raw_args=None):
	...
	dataset = PointCloudDataset("data/features_multiple_chairs.pt", "data/bbox_labels_multiple_chairs.pt", "data/loss_mask_multiple_chairs.pt")
	...
	# ###################################################################
	# # Choose loss weights
	# w1 = torch.tensor(2.0).to(device) #1
	...
	for i, data in enumerate(dataloader_cp, 0):
		...
		# prior.init_xi()
		# outputs = model_cp(inputs)
		scaling_cp = scale_prediction(boxes_3detr, boxes_gt, loss_mask, 0.97) #for coverage of 0.95 w.p. 0.99 
	...
	# mean_diff_pac = 0
	# mean_diff_cp = 0
	# diff_gt_cp = torch.zeros(len(dataloader_test),2)
	# diff_gt_pac = torch.zeros(len(dataloader_test),2)
	# len_gt = torch.zeros(len(dataloader_test),3)
	...
	# 	len_cp = boxes_cp[:,:,1,0:2][:,:,None,:] - boxes_cp[:,:,0,0:2][:,:,None,:]
	# 	len_gt[i,0] = torch.max(boxes_gt[0,0,0,0], boxes_gt[0,0,1,0]) - torch.min(boxes_gt[0,0,0,0],boxes_gt[0,0,1,0])
	# 	len_gt[i,1] = torch.max(boxes_gt[0,0,0,1], boxes_gt[0,0,1,1]) - torch.min(boxes_gt[0,0,0,1],boxes_gt[0,0,1,1])
	# 	len_gt[i,2] = torch.max(boxes_gt[0,0,0,1], boxes_gt[0,0,1,2]) - torch.min(boxes_gt[0,0,0,1],boxes_gt[0,0,1,2])
		
	# 	boxes_pac = outputs + boxes_3detr
	# 	idx = torch.where(loss_mask[0,:]==1)
	# 	if len(idx) > 0:
	# 		diff_gt_cp[i,0] = torch.mean(boxes_cp[:,idx[0],1,0] - boxes_cp[:,idx[0],0,0])-len_gt[i,0]
	# 		diff_gt_cp[i,1] = torch.mean(boxes_cp[:,idx[0],1,1] - boxes_cp[:,idx[0],0,1])-len_gt[i,1]
	...
	# fig, ax = plt.subplots(2, 1, tight_layout=True, sharex=True, sharey=True,)
	# plt.xlabel("Difference between predicted and ground length of boxes along x/y-axis (upper/lower plot) ")
	# ax[0].hist(diff_gt_pac[:,0].detach().cpu().numpy(), bins = 25)
	# ax[1].hist(diff_gt_pac[:,1].detach().cpu().numpy(), bins = 25)
	# plt.ylabel("Frequency (100 environments)")
	# # plt.xlabel("y-axis difference pred and GT length")
	# # Show plot
	# plt.show()

...
class FMTPlanner_dynamics():
    def __init__(
        ...
        self.ux_max = ux_max/dx
        self.ux_min = ux_min/dx
    ...
    def check_ICS(self, node: np.ndarray)-> bool:
        """
        Check if node is in ICS

        Args:
            node (np.ndarray): Node to check

        Returns:
            bool: True if no ICS violation
        """
        brake_dist = 0.5*self.ux_max**2/self.amax

        # if dynamics:

        # max deceleration when going straight
        x0,y0 = node[0]+brake_dist*np.cos(node[2]),node[1]+brake_dist*np.sin(node[2])
        violation_dist0 = self.obstacles_tree.query([x0,y0])[0]

        # max deceleration when turning left
        x1,y1 = node[0]+brake_dist*np.cos(node[2]+np.pi/2),node[1]+brake_dist*np.sin(node[2]+np.pi/2)
        violation_dist1 = self.obstacles_tree.query([x1,y1])[0]

        # max deceleration when turning right
        x2,y2 = node[0]+brake_dist*np.cos(node[2]-np.pi/2),node[1]+brake_dist*np.sin(node[2]-np.pi/2)
        violation_dist2 = self.obstacles_tree.query([x2,y2])[0]

        # else:
        #     violation_dist0 = self.obstacles_tree.query([node[0],node[1]+brake_dist])[0]
        #     violation_dist1 = self.obstacles_tree.query([node[0]-brake_dist,node[1]])[0]
        #     violation_dist2 = self.obstacles_tree.query([node[0]+brake_dist,node[1]])[0]
        
        return bool(min(violation_dist0,violation_dist1,violation_dist2) > self.rr)